...
public class Trace {
   ...
   */
  public Trace(PVector position, int nSteps, float speed) {
    this.nSteps = nSteps;
    ...
    float initAng = random(TWO_PI);
    float noiseSeed = random(1000);

    for (int step = 1; step < this.nSteps; step++) {
      float ang = initAng + TWO_PI * (noise(noiseSeed + NOISE_FACTOR * step) - 0.5f);
      pos = new PVector(pos.x + speed * cos(ang), pos.y + speed * sin(ang));
  ...
  public boolean hasValidTrajectory(boolean[] similarColor, boolean[] visitedPixels, PImage originalImg) {
    ...
    if (insideCounter > 1) {
      redStDev = sqrt((redSqSum - redSum * redSum / insideCounter) / (insideCounter - 1));
      greenStDev = sqrt((greenSqSum - greenSum * greenSum / insideCounter) / (insideCounter - 1));
      blueStDev = sqrt((blueSqSum - blueSum * blueSum / insideCounter) / (insideCounter - 1));
    }
  ...
  public boolean calculateColors(int[] maxColorDiff, boolean[] similarColor, PImage originalImg, PGraphics canvas,
    ...
    for (int step = 0; step < nSteps; step++) {
      ...
      alphas[step] = constrain((int) alpha, 0, 255);
    ...
    float hueAverage = hue(averageColor);
    float saturationAverage = saturation(averageColor);
    float brightnessAverage = brightness(averageColor);
    float noiseSeed = random(1000);
    ...
    for (int bristle = 0; bristle < nBristles; bristle++) {
      ...
      float deltaBrightness = BRIGHTNESS_RELATIVE_CHANGE * brightnessAverage
          * (noise(noiseSeed + 0.4f * bristle) - 0.5f);
      colors[0][bristle] = color(hueAverage, saturationAverage,
          constrain(brightnessAverage + deltaBrightness, 0, 255), 255);
    }

    colorMode(RGB, 255);

    // Extend the colors to the step where the mixing starts
    int mixStartingStep = constrain(TYPICAL_MIX_STARTING_STEP, 1, nSteps);
