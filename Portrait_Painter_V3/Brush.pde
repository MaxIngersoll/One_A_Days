...
public class Brush {
  ...
  public Brush(float size) {
    ...
    this.nBristles = (int) (size * random(1.6f, 1.9f));
    ...
    int nElements = Math.round(sqrt(2 * bristleLength));
    ...
    for (int bristle = 0; bristle < this.nBristles; bristle++) {
      ...
      this.bOffsets[bristle] = new PVector(size * random(-0.5f, 0.5f),
          BRISTLE_VERTICAL_NOISE * random(-0.5f, 0.5f));
  ...
  public void update(PVector newPosition, boolean updateBristleElements) {
    ...
    float directionAngle = HALF_PI
        + atan2(yNewAverage - averagePosition.y, xNewAverage - averagePosition.x);
  ...
  private void updateBristlePositions(float directionAngle) {
    if (positionsHistory.size() >= POSITIONS_FOR_AVERAGE - 1) {
      ...
      float cos = cos(directionAngle);
      float sin = sin(directionAngle);
      ...
      for (int bristle = 0; bristle < nBristles; bristle++) {
        ...
        float x = offset.x + bristleHorizontalNoise * (noise(noisePos + 0.1f * bristle) - 0.5f);
        float y = offset.y;
  ...
  public void paintOnScreen(int[] colors, int alpha) {
    if (positionsHistory.size() == POSITIONS_FOR_AVERAGE) {
      ...
      // Paint the bristles
      for (int bristle = 0; bristle < nBristles; bristle++) {
        bristles[bristle].paintOnScreen((colors[bristle] & 0x00ffffff) | alpha);
      }
    }
  }
